...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        evidenceVariablesSet = set(evidenceDict)
        queryVariablesSet = set(queryVariables)
        eliminationVariables = (bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet
        
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for var in eliminationOrder:
            currentFactorsList, Factor_joining = joinFactorsByVariable(currentFactorsList, var)
            
            if len(Factor_joining.unconditionedVariables()) != 1:
                Marginalized_Joint_elimination = eliminate(Factor_joining, var)
                currentFactorsList.append(Marginalized_Joint_elimination)

        new_Joint_factor = joinFactors(currentFactorsList)
        return normalize(new_Joint_factor)
