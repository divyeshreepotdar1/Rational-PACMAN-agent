        ...
        for wall in gameState.getHouseWalls(house):
            ...
            # and finally over assignments to (wall color, ghost house position, food house position) triples.                 
             for assignment in factor.getAllPossibleAssignmentDicts():
                probability = 0
                if assignment[FOOD_HOUSE_VAR] == HOUSE_VALS[counter]:
                        if assignment[wall] == "red":
                            probability = PROB_FOOD_RED
                        elif assignment[wall] == "blue":
                ...
                elif assignment[GHOST_HOUSE_VAR] == HOUSE_VALS[counter]:
                            if assignment[wall] == "red":
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    variable_elimination_method = inference.inferenceByVariableElimination(bayesNet, "foodHouse", evidence, eliminationOrder)
    possible_values = variable_elimination_method.getAllPossibleAssignmentDicts()
    for assigned_position in possible_values:
         probability = variable_elimination_method.getProbability(assigned_position)

         if probability < max_prob:
             continue
         max_prob = probability
         most_probable_location = assigned_position

    return most_probable_location   
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        joined_distribution = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)

        for possible_assign in joined_distribution.getAllPossibleAssignmentDicts():
            
            if possible_assign[HOUSE_VARS[0]] == TOP_LEFT_VAL:
                leftExpectedValue += joined_distribution.getProbability(possible_assign) * WON_GAME_REWARD  
                rightExpectedValue += joined_distribution.getProbability(possible_assign) * GHOST_COLLISION_REWARD 
            elif possible_assign[HOUSE_VARS[0]] == TOP_RIGHT_VAL:
                leftExpectedValue += joined_distribution.getProbability(possible_assign) * GHOST_COLLISION_REWARD  
                rightExpectedValue += joined_distribution.getProbability(possible_assign) * WON_GAME_REWARD 
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for exploration_pro in self.getExplorationProbsAndOutcomes(evidence):
            evidence.update(exploration_pro[1])
            expectedValue = expectedValue + exploration_pro[0] * max(self.computeEnterValues(evidence, enterEliminationOrder))
